...
**Implementation Update**: The script now uses live Azure DevOps API connectivity by default, with deterministic mock data available via the `--mock` flag for testing and demonstration purposes.
...
**Independent Test**: Execute the script with one org URL, a valid PAT (via arg or env), begin/end dates, and `--output`. By default (live mode), confirm data is fetched from real Azure DevOps APIs. With `--mock` flag, confirm deterministic mock data is generated.
...
3. **Given** the `--mock` flag is used, **When** I run the script, **Then** deterministic mock data is generated instead of fetching from real Azure DevOps APIs.
4. **Given** live mode (default) is used but `requests` module is not available, **When** I run the script, **Then** the script exits with error guidance to install the requests module.
...
**Independent Test**: Execute the script with `--jobs_output` in addition to P1 inputs. Confirm a second CSV is produced with one row per job including job name/id, duration, assigned pool name/id, and references to the run/pipeline. In live mode (default), agent pool details are fetched from real Azure DevOps APIs. With `--mock` flag, deterministic mock data is used.

**Acceptance Scenarios**:

1. **Given** `--jobs_output` is provided, **When** I run the script, **Then** a job-level CSV is written and includes pool identifiers.
2. **Given** `--mock` and `--jobs_output` are used, **When** I run the script, **Then** deterministic mock agent pool information is generated and included in job records.
...
**Independent Test**: Provide multiple org URLs (via repeated flag or list input). Confirm the resulting pipeline-level CSV includes records across all orgs with clear org identifiers. Works in both live mode (default) and mock mode (`--mock`).

**Acceptance Scenarios**:

1. **Given** two org URLs, **When** I run the script, **Then** the pipeline CSV contains rows attributed to each org and totals reflect data from all organizations.
2. **Given** multiple org URLs and `--mock` flag, **When** I run the script, **Then** deterministic mock data is generated for all specified organizations and consolidated into unified CSV outputs.
...
- Large datasets: Script handles large datasets; outputs remain streaming-friendly.
- Rate limiting: Respect a user-provided `--delay` between requests to simulate polite API usage.
- **Live API failures**: In live mode (default), network issues, invalid PATs, or API errors result in non-zero exit with actionable error messages.
- **Missing dependencies**: If live mode is used (default) but `requests` module is not available, script exits with installation guidance.
- **API permissions**: In live mode, insufficient PAT permissions for projects/builds result in clear error messages indicating required permissions.
...
- **FR-005**: The script MUST honor `API_VERSION` defined in the script for selecting Azure DevOps API version semantics.
- **FR-006**: The script MUST support `--threads` for parallel project processing and `--delay` (milliseconds) to throttle requests.
...
- **FR-010**: Live API mode MUST be the default: real API calls are made by default. Mock mode MUST be available via `--mock` flag for testing with deterministic generated data.
- **FR-011**: By default, the script MUST make authenticated requests to real Azure DevOps APIs using the specified API_VERSION and handle common API failures gracefully.
- **FR-012**: The script MUST validate that the `requests` module is available when running in live mode (default), exiting with clear installation guidance if missing.
...
- **SC-005**: Live API mode (default) successfully connects to real Azure DevOps instances and produces CSV output with real data.
...
- **SC-007**: Both mock (`--mock`) and live modes produce identical output structure with identical column ordering and data types.

## Assumptions

- By default, the script connects to real Azure DevOps APIs and requires valid credentials and network connectivity.
- Mock mode is available via `--mock` flag for testing and development without live API calls; generated results are deterministic.
- Deterministic outputs: same inputs produce the same CSV structure; mock mode produces identical content, live mode reflects real-time API data.
...
- Live API mode (default) requires the `requests` Python module to be available; clear error guidance is provided if missing.
